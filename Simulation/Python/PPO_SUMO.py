...
env = make_vec_env("SumoEnv-v0", n_envs=1)

# Train the agent
model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=logdir, device="cpu")

# Train the agent
model.learn(total_timesteps=10000, tb_log_name="PPO_sumo")
model.save("ppo_sumo")
...
waiting_times = []  # List to store average waiting time at each step
...
while steps < 1000:
    ...
    # print(f"steps: {steps}, obs: {obs}")
    print(
        f"Step: {steps}, Reward: {reward}, Obs: {obs}, Action: {action}, Done: {done}")
    steps += 1

    waiting_times.append(obs.item(0))  # Append average waiting time from obs
    np.set_printoptions(suppress=True, precision=3, floatmode="fixed")
